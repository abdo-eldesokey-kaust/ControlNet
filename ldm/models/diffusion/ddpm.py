...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        for k in self.ucg_training: # Empty
...
class LatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cond_stage_key = cond_stage_key  # txt
        ...
        self.instantiate_first_stage(first_stage_config)  # Initialize the Autoencoder and freeze it 
    ...
    def get_input(
        ...
        if bs is not None:  # None
            x = x[:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)  # DiagonalGaussianDistribution
        z = self.get_first_stage_encoding(encoder_posterior).detach()  # B x 4 x 64 x 64
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox", "txt"]:
                    xc = batch[cond_key]  # Text prompts
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c = self.get_learned_conditioning(xc.to(self.device)) # B x 77 x 768
            else:
                c = xc
            if bs is not None:
                c = c[:bs]
    ...
    def shared_step(self, batch, **kwargs):
        x, c = self.get_input(batch, self.first_stage_key)  # z, dict of text encoding and edge map encoding 
        loss = self(x, c) # Call forward()
        return loss
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        # x_start is the latent 
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, cond) # B x 4 X H/4 x W/4
...
class DiffusionWrapper(pl.LightningModule):  # => self.model
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.sequential_cross_attn = diff_model_config.pop("sequential_crossattn", False)
        self.diffusion_model = instantiate_from_config(diff_model_config)  # ControlledUnetModel
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [None, "concat", "crossattn", "hybrid", "adm", "hybrid-adm", "crossattn-adm"]

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None): # Not used
